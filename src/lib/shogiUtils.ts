...
export const isSameMove = (m1?: Move | null, m2?: Move | null): boolean => {
  if (!m1 || !m2) return false;
  const fromMatch = (!m1.from && !m2.from) ||
    (m1.from?.x === m2.from?.x && m1.from?.y === m2.from?.y);
  const toMatch = m1.to.x === m2.to.x && m1.to.y === m2.to.y;
  const pieceMatch = (m1.piece || undefined) === (m2.piece || undefined);
  const promoteMatch = Boolean(m1.promote) === Boolean(m2.promote);
  return fromMatch && toMatch && pieceMatch && promoteMatch;
};

export const matchesMoveHistory = (sequence: Move[], history: Move[]): boolean => {
  if (!sequence || !history || sequence.length < history.length) return false;
  for (let i = 0; i < history.length; i++) {
    if (!isSameMove(sequence[i], history[i])) return false;
  }
  return true;
};
